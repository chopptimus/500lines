...
One problem with this: in the context of StackExchange dumps, it’s
difficult to motivate the need for incremental index updates, but most
of the time, incremental index updates are not optional, and they can
complicate a lot of things.  So I’d like to show that they can be
...
So I figured the thing to do is to make it a generic text-search
program like Glimpse, storing the index in a directory somewhere up
the parent hierarchy like `.git`, with a list of filenames and mtimes?
Then I can just reindex files when they change.  `grep -r e1000e
...
You could binary-search this file to find all the lines that begin
...
There’s a chunk size tradeoff: smaller chunks mean more filesystem
...
indices in RAM before writing them out to disk.  This engine <!-- XXX
chispa? --> by default builds up 4 million postings in RAM which takes
...
Since we’re restricting ourselves to essentially sequential access for
...
It turns out there’s a middle ground that works pretty well, although
...
This engine does a very crude ranking: it returns the newest files
first:
...
So that’s the sequence of postings being broken into 1048576-item
...
But `build_index` also invoked `merge_segments(index_path,
...
As a sort of in-between alternative, you could use a high-speed
